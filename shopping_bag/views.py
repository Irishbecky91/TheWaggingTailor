...
def add_to_shopping_bag(request, item_id):
    ...
    if size:
        ...
        if item_id in list(shopping_bag.keys()):
            ...
            messages.success(request, f'You updated the number of {product.name} in your shopping bag to {shopping_bag[item_id]}')
        else:
            shopping_bag[item_id] = quantity
            messages.success(request, f'You added {product.name} to your shopping bag')
    ...
    if size:
        if quantity > 0:
            ...
            if not shopping_bag[item_id]['items_by_size']:
                ...
                messages.success(request, f'You removed {size.upper()} {product.name} from your shopping bag')
    else:
        if quantity > 0:
            shopping_bag[item_id] = quantity
            messages.success(request, f'You updated the number of {product.name} in your shopping bag to {shopping_bag[item_id]}')
